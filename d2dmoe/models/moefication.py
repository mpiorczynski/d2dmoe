...
from d2dmoe.models.moe import MoEBertConfig, MoELayer, MoEViTConfig, Router, TopKGating
...
class MoEficationRouter(Router):
...
class MoEficationLayer(MoELayer):
    def __init__(self, hidden_size: int, num_experts: int, expert_size: int, activation: nn.Module):
        ...
        self.router = MoEficationRouter(hidden_size, num_experts)
        self.gate = TopKGating(num_experts)

    def set_for_eval(self, k: int):
        self.enable_gating()
        self.gate.set_k(k)


class MoEficationBertForSequenceClassification(BertForSequenceClassification):
    ...
    def replace_ffns_with_moes(self):
        ...
        for i, layer_name in tqdm(enumerate(modules_to_moefy), desc="Replacing FFNs with MoEs"):
            ...
            moefied_layer = replace_layer_with_moe(
                original_layer, MoEficationLayer, self.num_experts, self.expert_size
            )
...
class MoEficationViTForImageClassification(ViTForImageClassification):
